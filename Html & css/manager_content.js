    ...
    try {
      ...
      window.alert("Error fetching data. Please try again.");
          ...
          async function fetchAllStaff() {
            try {
                const response = await fetch('/api/All_staff');
                if (!response.ok) throw new Error("Network response was not ok");
                
                const data = await response.json();
                if (!Array.isArray(data)) throw new Error("Unexpected data format");
          
                const allStaffTable = document.getElementById('All_staffTable');
                const tbody = document.getElementById('all_staffTablebody');
                ...
                data.forEach(staff => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${staff.IFSCE_CODE}</td>
                    `;
                    tbody.appendChild(row);
                });
          
                allStaffTable.style.display = 'table';
            } catch (error) {
                console.error('Error fetching all staff data:', error);
            }
          }
          
        // Function to fetch and display Branch Staff data based on IFSC code
        const branch_staffIN = document.getElementById('branch_staffIN');
        const branch_staffBtn = document.getElementById('branch_staffBtn');

        branch_staffIN.style.display ='none';
        branch_staffBtn.style.display = 'none';

        async function fetchBranchStaff() {
            const ifsce = branch_staffIN.value.trim();
            if (!ifsce) {
            ...
            try {
                const response = await fetch(`/api/branch_staff/${ifsce}`);
                const data = await response.json();
                
                const branchStaffTable = document.getElementById('branch_staffTable');
                const tbody = document.getElementById('branch_staffTbody');
                tbody.innerHTML = ''; // Clear previous rows
                ...
                branchStaffTable.style.display = 'table';
            } catch (error) {
                console.error('Error fetching branch staff data:', error);
            }
        }
...
actionDropdown.addEventListener("change", () => {
  ...
  branch_staffBtn.style.display = 'none';
  Acc_form.style.display = 'none';
  branchStaffTable.style.display= 'none';
  }
  ...
  branchStaffTable.style.display= 'none';
    ...
    countButton.style.display ='none';
    countTable.style.display = 'none';
    allStaffTable.style.display ='none';
    branchStaffTable.style.display= 'none';
    branch_staffIN.style.display ='none';
    branch_staffBtn.style.display = 'none';
    Acc_form.style.display = 'none';
  ...
  else if(selectedAction === 'AllStaffs'){
    countInput.style.display = 'none';
    countButton.style.display ='none';
    cus_table.style.display = 'none';
    ocTable.style.display ='none';
  ...
  else if(selectedAction === 'BranchStaffs'){
    branch_staffIN.style.display ='inline';
        branch_staffBtn.style.display = 'inline';
        countInput.style.display = 'none';
        countButton.style.display ='none';
        cus_table.style.display = 'none';
        ocTable.style.display ='none';
...
branch_staffBtn.addEventListener("click",fetchBranchStaff);
